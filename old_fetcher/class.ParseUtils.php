...
class ParseUtils {
	...
	private $antigate_captcha_id = false;
	public $antigate_key = false;
	public $antigate_domain = 'antigate.com';
	public $log_func = 'parseutils_log';
	
	
	public function log($s) {
		if(is_callable($this->log_func)) {
			call_user_func_array($this->log_func, array($s, &$this));
		}
	}
	...
	*/
	            ...
	            )
	{
		...
		{
			$this->log('antigate: file "'.$filename.'" not found!');
			return false;
		}
		
	    $postdata = array(
	        'method'    => 'post', 
	        'key'       => $this->antigate_key, 
	    ...
	    $ch = curl_init();
	    curl_setopt($ch, CURLOPT_URL,             "http://".($this->antigate_domain)."/in.php");
	    ...
	    if (curl_errno($ch)) 
	    {
	    	$this->log('antigate: fetch error: "'.curl_error($ch).'"');
	        return false;
	    }
	    curl_close($ch);
	    if (strpos($result, "ERROR")!==false)
	    {
	    	$this->log('antigate: server returned error: "'.$result.'"');
	        return false;
	    }
	    else
	    {
	        $ex = explode("|", $result);
	        $captcha_id = $ex[1];
	        $this->antigate_captcha_id = $captcha_id;
	    	if ($is_verbose) $this->log("antigate: captcha sent, got captcha ID $captcha_id");
	        $waittime = 0;
	        if ($is_verbose) $this->log("antigate: waiting for $rtimeout seconds");
	        sleep($rtimeout);
	        while(true)
	        {
	            $result = file_get_contents("http://".($this->antigate_domain)."/res.php?key=".($this->antigate_key).'&action=get&id='.$captcha_id);
	            if (strpos($result, 'ERROR')!==false)
	            {
	            	$this->log("antigate: server returned error: $result");
	                return false;
	            ...
	            {
	            	if ($is_verbose) $this->log("antigate: captcha is not ready yet");
	            	$waittime += $rtimeout;
	            	if ($waittime>$mtimeout) 
	            	{
	            		if ($is_verbose) $this->log("antigate: timelimit ($mtimeout) hit");
	            		break;
	            	}
	        		if ($is_verbose) $this->log("antigate: waiting for $rtimeout seconds");
	            	sleep($rtimeout);
	            ...
	            else
	            {
	            	$ex = explode('|', $result);
	            	if (trim($ex[0])=='OK') {
	            		$this->log("antigate: get captcha code \"".trim($ex[1])."\"");
	            		return trim($ex[1]);
	            	}
	            }
	        }
	...
	public function antigate_reportbad() {
		return file_get_contents('http://'.($this->antigate_domain).'/res.php?key='.($this->antigate_key).'&action=reportbad&id='.($this->antigate_captcha_id));
	}
	 ...
	 * @version  4.0.14
	 */
	public function strip_tags_smart(
	    /*string*/ $s,
	...
	public function cut_str($s, $s1, $s2) {
		$spos = strpos($s, $s1);
		if($spos === false) return false;
		$spos2 = strpos($s, $s2, $spos);
		if($spos2 === false) return false;
		$spos2 = $spos2 + strlen($s2);
		return substr($s, $spos, $spos2-$spos);
	}
	
	public function parse_num($s) {
		$s = str_replace(',', '.', $s);
